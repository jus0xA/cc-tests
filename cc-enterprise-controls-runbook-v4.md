...
```powershell
$TestRoot = "$env:USERPROFILE\Tools\cc-controls-test"
...
- **Controls 2–7** are Claude Code session tests. For each test:
  1. Open a terminal, `cd` to `%USERPROFILE%\Tools\cc-controls-test\workdir`
...
$LogFile = "$env:USERPROFILE\Tools\cc-controls-test\evidence\control1-standard.txt"
...
$LogFile = "$env:USERPROFILE\Tools\cc-controls-test\evidence\control1-admin.txt"
...
$LogFile = "$env:USERPROFILE\Tools\cc-controls-test\evidence\control1-bypass.txt"
...
> All tests below: `cd %USERPROFILE%\Tools\cc-controls-test\workdir`, then launch `claude`.
...
```powershell
'Invoke-WebRequest -Uri "https://httpbin.org/get" -UseBasicParsing' | Out-File "$env:USERPROFILE\Tools\cc-controls-test\workdir\egress.ps1" -Encoding UTF8
```
...
Pre-step:
```powershell
New-Item -ItemType Directory -Force "$env:USERPROFILE\Tools\cc-controls-test\workdir\.claude" | Out-Null
@'
{
  "permissions": {
    "disableBypassPermissionsMode": "",
...
'@ | Out-File "$env:USERPROFILE\Tools\cc-controls-test\workdir\.claude\settings.json" -Encoding UTF8
```
...
Clean up:
```powershell
Remove-Item "$env:USERPROFILE\Tools\cc-controls-test\workdir\.claude" -Recurse -Force
```
...
'@ | Out-File "$env:USERPROFILE\Tools\cc-controls-test\workdir\.mcp.json" -Encoding UTF8
```
...
Clean up:
```powershell
Remove-Item "$env:USERPROFILE\Tools\cc-controls-test\workdir\.mcp.json" -Force
```
...
New-Item -ItemType Directory -Force "$env:USERPROFILE\Tools\cc-controls-test\workdir\.claude" | Out-Null
'{"mcpServers":{"project-bypass":{"command":"python3","args":["-c","print(1)"]}}}' | Out-File "$env:USERPROFILE\Tools\cc-controls-test\workdir\.claude\settings.json" -Encoding UTF8
```
...
```
'@ | Out-File "$env:USERPROFILE\Tools\cc-controls-test\workdir\CLAUDE.md" -Encoding UTF8
```
...
Clean up:
```powershell
Remove-Item "$env:USERPROFILE\Tools\cc-controls-test\workdir\CLAUDE.md" -Force
```
...
Remove-Item "$env:USERPROFILE\Tools\cc-controls-test\workdir" -Recurse -Force -ErrorAction SilentlyContinue
...
Write-Host "[+] Cleanup complete. Evidence preserved in: $env:USERPROFILE\Tools\cc-controls-test\evidence" -ForegroundColor Green
```
